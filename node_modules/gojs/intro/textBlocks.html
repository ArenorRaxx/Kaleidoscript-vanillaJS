...
<!-- Copyright 1998-2022 by Northwoods Software Corporation. -->    <title> GoJS TextBlocks -- Northwoods Software </title>
...
For this reason, if you need objects to measure precisely and consistently across all browsers (such as when testing),
...
For example, if you have a TextBlock with natural size inside an <a>Panel.Auto</a> Panel,
set the size on that Auto Panel.
</p>
...
<script>goCode("multilineTextBlocks", 600, 300)</script>

<h2 id="Indenting">Indenting</h2>
<p>
  Each line of text is normally trimmed of leading and trailing spaces before rendering.
  If you wish to indent a line of text by preserving leading spaces, start the line with the
  zero-width space character: <code>\u200B</code>.
...
<pre class="lang-js" id="indent"><code>
  diagram.add(
    $(go.Part, "Vertical",
      $(go.TextBlock, { text: "left aligned\n\u200B  indent two\n\u200B    indent four",
                        background: "lightgreen", margin: 2, width: 150 }),
      $(go.TextBlock, { text: "\u200B  This is an indented paragraph consisting of lots of text that wraps naturally.",
                        background: "lightgreen", margin: 2, width: 150 })
    ));
</code></pre>
<script>goCode("indent", 600, 200)</script>
<p>
  You can also use a <code>\u200B</code> character if you want to preserve spaces at the end of a line.
</p>
