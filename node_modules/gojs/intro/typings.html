...
<!-- Copyright 1998-2022 by Northwoods Software Corporation. -->    <title> GoJS Typings in JavaScript or TypeScript </title>
...
<h1>GoJS typings for TypeScript and JavaScript</h1>

<p>
  This page assumes you will be using NPM to get the TypeScript typings for use in either JavaScript or TypeScript.
  This page also assumes you have a npm project already, if you do not, you may need to run <code>npm init</code>.
</p>

<p>
  In a TypeScript environment, GoJS typings will appear automatically if you have the <code>gojs</code> npm package installed.
  These typings will offer auto-completion, inline documentation, and value error checking.
  For example when defining a shape, it will auto-complete <code>strokeWidth</code> and display documentation:
</p>

<img class="w-full my-1 md:p-8 max-w-5xl" src="images/types01.png"></img>

<p>
  And since <code>strokeWidth</code> does not exist on <a>Node</a>, it will point out an error:
</p>

<img class="w-full my-1 md:p-8 max-w-5xl" src="images/types02.png"></img>

<h1>Typings in JavaScript files</h1>

<p>
  If you have a compatible editor, such as VS Code, you can use TypeScript typings in plain JavaScript,
  provided you have TypeScript installed.
  The easiest way to use typings is to install the GoJS npm package,
  even if you are referencing the <code>go.js</code> library from somewhere else, such as a CDN.
  And of course you must also install TypeScript in your project, or globally.
</p>
<pre class="lang-bash"><code>  npm i typescript --save-dev
  npm i gojs --save-dev
</code></pre>
...
<pre class="lang-bash"><code>  npx tsc --init
</code></pre>

In your <code>tsconfig.json</code> file, you will have to enable the <code>allowJs</code> and <code>checkJs</code> options:
...
<p>
  Typescript will then note that "go" is not defined.
  The best course of action depends on whether the JavaScript file is a module.
</p>

<h3>Plain JavaScript files</h3>
<p>
  To make sure the symbol is accessible, you must add this triple-slash directive to the top of your <code>.js</code> files:
  This allows type checking in JS files without defining "go".
</p>
<pre class="lang-js"><code>/// &lt;reference path="./node_modules/gojs/release/go.d.ts" /&gt;
</code></pre>
...
<p>
  If you are using JavaScript modules, a triple-slash directive is not necessary --
  you can access the <code>go</code> object through import and TypeScript typings will work:
</p>
<pre class="lang-js"><code>import * as go from 'gojs';</code></pre>

<h1 class="mt-6">Typings without npm</h1>

<p>
  Every release of GoJS contains the <a href="../release/go.d.ts" target="_blank">go.d.ts</a> file, in the <code>release/</code> folder.
  You can view and copy these files and others from <a href="https://github.com/NorthwoodsSoftware/gojs" target="_blank">GitHub</a>.
  See the <a href="../download.html">download page</a> for other options.
</p>
      </div>
    </div>
