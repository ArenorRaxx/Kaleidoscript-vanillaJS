...
<!-- Copyright 1998-2022 by Northwoods Software Corporation. -->    <title> GoJS Coordinate Systems-- Northwoods Software </title>
...
<p>
A <a>Diagram</a> uses two major coordinate systems when drawing <a>Part</a>s: document coordinates and view coordinates.
...
Thus the <a>Point</a> that may be saved for a Node's location in the model's node data object are normally in document coordinates:
</p>
...
<p>
The union of all of the Nodes and Links in a Diagram form the <a>Diagram.documentBounds</a>.
This <a>Rect</a> has values that are in document coordinates.
Depending on where the Nodes and Links are, the document bounds may cover a very large area.
The range of the document bounds might be entirely positive x and y values, or that might be entirely negative,
or more likely they may cover both negative and positive values.
</p>
<p>
...
<p>
Points in the canvas are in view coordinates, which are independent of document coordinates.
View coordinates are distances from the top-left corner of the canvas in device-independent pixels.
...
However very few properties or method arguments or return values are in view coordinates -- almost everything is in document
